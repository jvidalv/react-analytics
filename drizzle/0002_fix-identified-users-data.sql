-- Fix identified users data extraction
-- Problem: Materialized views only checked properties->'data' path
-- Some identify events store data at properties root level
-- Solution: Check both nested and top-level paths with COALESCE fallbacks
...
-- Recreate analytics_identified_users_mv with fixed data extraction
...
SELECT
  ...
  COALESCE(
    ...
    NULLIF(TRIM(COALESCE(
      led.properties->'data'->>'firstName',
      led.properties->>'firstName',
      ''
    ) || ' ' || COALESCE(
      led.properties->'data'->>'lastName',
      led.properties->>'lastName',
      ''
    )), '')
  ) as name,
  COALESCE(
    led.properties->'data'->>'email',
    led.properties->>'email'
  ) as email,
  COALESCE(
    led.properties->'data'->>'avatarUrl',
    led.properties->'data'->>'avatar',
    led.properties->>'avatarUrl',
    led.properties->>'avatar'
  ) as avatar,
  CASE
...
-- Recreate analytics_test_identified_users_mv with fixed data extraction
