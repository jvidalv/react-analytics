-- Fix materialized views to only use 'identify' type events for user properties
-- Problem: latest_event_data CTE was selecting ANY event type (navigation, action, etc.)
-- Those events don't contain user data (name, email, avatar)
-- Solution: Filter for type = 'identify' events only
...
-- Recreate analytics_identified_users_mv with identify events only
...
WITH first_last_seen AS (
  ...
  ORDER BY api_key, identify_id, date DESC
),
latest_event_data AS (
  ...
  WHERE a.user_id IS NOT NULL AND a.type = 'identify'
...
-- Recreate analytics_test_identified_users_mv with identify events only
