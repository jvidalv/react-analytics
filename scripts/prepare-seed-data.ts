...
function prepareFile(inputFile: string, outputFile: string, tableName: string, knownApiKey: string) {
  ...
  const placeholder = tableName === 'analytics' ? '{{API_KEY}}' : '{{API_KEY_TEST}}';
  ...
  const occurrences = (content.match(new RegExp(knownApiKey.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), "g")) || [])
    .length;
